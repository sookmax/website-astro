---
title: "From Data Analyst to Frontend Engineer at AgileSoDA"
description: "My first engineering job. Started out as a data analyst, but my growing interest in systems and software development led me to transition into a full-time engineer role, eventually specializing in frontend. A pivotal experience that shaped my career path."
type: "Work"
from: 2017-04
to: 2020-12
at: "AgileSoDA"
...
## Starting Out: Life as a Data Analyst
...
This was my third and final job as a data analyst—and the one that changed everything.
...
When the data doesn’t contain any meaningful signals, no amount of modeling or data processing will improve the outcome—it’s still garbage in, garbage out. And what we often mistook for a “signal” turned out to be noise or even errors in the pipeline.
...
Our team had a dedicated engineer for this, but they rarely showed up at the client site—so I figured it out myself.

That initiative—and my growing curiosity for engineering—paved the way for a role change that would shape the rest of my career.
...
I didn't always have answers, but these questions sharpened my technical instincts. And they made me hungry to specialize further.
...
What pushed me toward frontend was—ironically—a terrible frontend codebase.

The app was bootstrapped with [this now-defunct boilerplate](https://github.com/erikras/react-redux-universal-hot-example), meant for isomorphic rendering. But we were only doing client-side rendering—meaning we didn't need all the overhead of Node.js and SSR, yet we still had a complex server and bloated architecture. I gave an internal presentation just to express my frustration.

Fortunately, I was later asked to build the UI for a brand new product _from scratch_. I was thrilled. Finally, I could:

- Use modern React with hooks (no more class components!)
- Pick sensible libraries: [Blueprint.js](https://blueprintjs.com/docs/) and [TanStack Table](https://tanstack.com/table/latest) for UI, [ECharts](https://echarts.apache.org/en/index.html) and [react-chartjs-2](https://react-chartjs-2.js.org/) for visualizations
- Avoid unnecessary complexity.

But as excited as I was about hooks, I quickly ran into issues with `useEffect()`. Some effects grew so convoluted that I had to sprinkle in `console.log()` just to understand them. I overused them due to my limited understanding of the React rendering model, and in the end, some of my own `useEffect()`s were so fragile I was afraid to touch them.
